...
const createBindingUsagesTableData = data => {
  if (!data.length) return [];
...
export default function ServiceInstanceBindings({
  ...
  if (serviceCatalogAddonsBackendModuleExists) {
    bindingUsagesTableData = createBindingUsagesTableData(
      serviceInstance.serviceBindingUsages,
    );
  }

  const createBindingsTableData = data => {
    if (!data.length) return [];
  ...
  const bindingsTableData =
    createBindingsTableData(serviceInstance.serviceBindings.items) || [];

  return (
    <ServiceInstanceBindingsWrapper>
      <Tabs defaultActiveTabIndex={defaultActiveTabIndex} callback={callback}>
        {serviceCatalogAddonsBackendModuleExists ? (
          <Tab
            title={
              <Tooltip
            ...
            addHeaderContent={<span>ADDBINDING</span>}
            status={status(
              serviceInstance.serviceBindingUsages,
              'status-service-binding-usage',
            )}
        ...
        <Tab
          ...
          addHeaderContent={<span>{createCredentialsContent}</span>}
          status={status(
            serviceInstance.serviceBindings.items,
            'status-service-binding',
          )}
